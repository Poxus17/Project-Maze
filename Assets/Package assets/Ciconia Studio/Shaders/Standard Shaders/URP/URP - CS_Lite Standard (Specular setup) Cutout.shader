Shader "Ciconia Studio/CS_Standard/URP/Lite/Standard (Specular setup)/Cutout"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag ( VertexOutput IN , half ase_vface : VFACE ) : SV_Target
			{
				...
				float2 paralaxOffset36_g489 = ParallaxOffset( temp_cast_0.x , _Parallax , ase_tanViewDir );
				float2 switchResult47_g489 = (((ase_vface>0)?(paralaxOffset36_g489):(0.0)));
				float2 Parallaxe380 = switchResult47_g489;
				float4 tex2DNode7_g554 = tex2D( _BaseMap, ( ( appendResult14_g555 + appendResult13_g555 ) + Parallaxe380 ) );
				...
				float4 temp_output_405_0 = CalculateContrast(_Brightness,float4( desaturateVar29_g554 , 0.0 ));
				float2 uv_DetailAlbedoMap = IN.ase_texcoord7.xy * _DetailAlbedoMap_ST.xy + _DetailAlbedoMap_ST.zw;
				float4 tex2DNode7_g560 = tex2D( _DetailAlbedoMap, uv_DetailAlbedoMap );
				float clampResult27_g560 = clamp( _DetailSaturation , -1.0 , 100.0 );
				float3 desaturateInitialColor29_g560 = ( _DetailColor * tex2DNode7_g560 ).rgb;
				float desaturateDot29_g560 = dot( desaturateInitialColor29_g560, float3( 0.299, 0.587, 0.114 ));
				float3 desaturateVar29_g560 = lerp( desaturateInitialColor29_g560, desaturateDot29_g560.xxx, -clampResult27_g560 );
				float4 AlbedoDetail153 = CalculateContrast(_DetailBrightness,float4( desaturateVar29_g560 , 0.0 ));
				float4 temp_cast_6 = (0.0).xxxx;
				float2 uv_DetailMask = IN.ase_texcoord7.xy * _DetailMask_ST.xy + _DetailMask_ST.zw;
				float4 tex2DNode27_g559 = tex2D( _DetailMask, uv_DetailMask );
				#if defined(_CHANNELSELECTIONMASK_RED)
				float staticSwitch28_g559 = tex2DNode27_g559.r;
				#elif defined(_CHANNELSELECTIONMASK_GREEN)
				float staticSwitch28_g559 = tex2DNode27_g559.g;
				#elif defined(_CHANNELSELECTIONMASK_BLUE)
				float staticSwitch28_g559 = tex2DNode27_g559.b;
				#elif defined(_CHANNELSELECTIONMASK_ALPHA)
				float staticSwitch28_g559 = tex2DNode27_g559.a;
				#else
				float staticSwitch28_g559 = tex2DNode27_g559.r;
				#endif
				float BaseColorAlpha46 = (( _InvertABaseColor )?( ( 1.0 - tex2DNode7_g554.a ) ):( tex2DNode7_g554.a ));
				float temp_output_43_0_g559 = BaseColorAlpha46;
				#if defined(_SOURCEMASK_DETAILMASK)
				float staticSwitch45_g559 = (( _InvertMask )?( ( 1.0 - staticSwitch28_g559 ) ):( staticSwitch28_g559 ));
				#elif defined(_SOURCEMASK_BASECOLORALPHA)
				float staticSwitch45_g559 = temp_output_43_0_g559;
				#elif defined(_SOURCEMASK_BOTH)
				float staticSwitch45_g559 = ( (( _InvertMask )?( ( 1.0 - staticSwitch28_g559 ) ):( staticSwitch28_g559 )) + temp_output_43_0_g559 );
				#else
				float staticSwitch45_g559 = (( _InvertMask )?( ( 1.0 - staticSwitch28_g559 ) ):( staticSwitch28_g559 ));
				#endif
				float4 temp_cast_7 = (staticSwitch45_g559).xxxx;
				float4 clampResult38_g559 = clamp( ( CalculateContrast(( _ContrastDetailMap + 1.0 ),temp_cast_7) + -_SpreadDetailMap ) , float4( 0,0,0,0 ) , float4( 1,1,1,0 ) );
				float4 Mask158 = (( _EnableDetailMask )?( ( clampResult38_g559 * _IntensityMask ) ):( temp_cast_6 ));
				float4 lerpResult365 = lerp( temp_output_405_0 , AlbedoDetail153 , Mask158);
				float4 blendOpSrc364 = temp_output_405_0;
				...
				float2 break26_g589 = uv_BumpMap;
				float2 appendResult14_g589 = (float2(( break26_g589.x * GlobalTilingX11 ) , ( break26_g589.y * GlobalTilingY8 )));
				float2 appendResult13_g589 = (float2(( break26_g589.x + GlobalOffsetX10 ) , ( break26_g589.y + GlobalOffsetY9 )));
				float3 unpack4_g588 = UnpackNormalScale( tex2D( _BumpMap, ( ( appendResult14_g589 + appendResult13_g589 ) + Parallaxe380 ) ), _BumpScale );
				unpack4_g588.z = lerp( 1, unpack4_g588.z, saturate(_BumpScale) );
				float3 temp_output_399_0 = unpack4_g588;
				float2 uv_DetailNormalMap = IN.ase_texcoord7.xy * _DetailNormalMap_ST.xy + _DetailNormalMap_ST.zw;
				float3 unpack4_g583 = UnpackNormalScale( tex2D( _DetailNormalMap, uv_DetailNormalMap ), _DetailNormalMapScale );
				unpack4_g583.z = lerp( 1, unpack4_g583.z, saturate(_DetailNormalMapScale) );
				float3 NormalDetail155 = unpack4_g583;
				float3 lerpResult137 = lerp( temp_output_399_0 , NormalDetail155 , Mask158.rgb);
				float3 lerpResult205 = lerp( temp_output_399_0 , BlendNormal( temp_output_399_0 , NormalDetail155 ) , Mask158.rgb);
				...
				float2 break26_g582 = uv_EmissionMap;
				float2 appendResult14_g582 = (float2(( break26_g582.x * GlobalTilingX11 ) , ( break26_g582.y * GlobalTilingY8 )));
				float2 appendResult13_g582 = (float2(( break26_g582.x + GlobalOffsetX10 ) , ( break26_g582.y + GlobalOffsetY9 )));
				float4 Emission110 = ( _EmissionColor * tex2D( _EmissionMap, ( ( appendResult14_g582 + appendResult13_g582 ) + Parallaxe380 ) ) * _EmissionIntensity );
				
				float2 uv_SpecGlossMap = IN.ase_texcoord7.xy * _SpecGlossMap_ST.xy + _SpecGlossMap_ST.zw;
				float2 break26_g585 = uv_SpecGlossMap;
				float2 appendResult14_g585 = (float2(( break26_g585.x * GlobalTilingX11 ) , ( break26_g585.y * GlobalTilingY8 )));
				float2 appendResult13_g585 = (float2(( break26_g585.x + GlobalOffsetX10 ) , ( break26_g585.y + GlobalOffsetY9 )));
				float4 tex2DNode3_g584 = tex2D( _SpecGlossMap, ( ( appendResult14_g585 + appendResult13_g585 ) + Parallaxe380 ) );
				float4 Specular41 = ( _SpecularColor * tex2DNode3_g584 * _SpecularIntensity );
				
				#if defined(_SOURCE_SPECULARALPHA)
				float staticSwitch31_g584 = ( tex2DNode3_g584.a * _Smoothness );
				#elif defined(_SOURCE_BASECOLORALPHA)
				float staticSwitch31_g584 = ( _Smoothness * BaseColorAlpha46 );
				#else
				float staticSwitch31_g584 = ( tex2DNode3_g584.a * _Smoothness );
				#endif
				float DetailBaseColorAlpha170 = ( _DetailColor.a * tex2DNode7_g560.a );
				float lerpResult387 = lerp( staticSwitch31_g584 , DetailBaseColorAlpha170 , Mask158.r);
				float Smoothness40 = lerpResult387;
				
				float2 uv_OcclusionMap = IN.ase_texcoord7.xy * _OcclusionMap_ST.xy + _OcclusionMap_ST.zw;
				float2 break26_g587 = uv_OcclusionMap;
				float2 appendResult14_g587 = (float2(( break26_g587.x * GlobalTilingX11 ) , ( break26_g587.y * GlobalTilingY8 )));
				float2 appendResult13_g587 = (float2(( break26_g587.x + GlobalOffsetX10 ) , ( break26_g587.y + GlobalOffsetY9 )));
				float blendOpSrc2_g586 = tex2D( _OcclusionMap, ( ( appendResult14_g587 + appendResult13_g587 ) + Parallaxe380 ) ).r;
				float blendOpDest2_g586 = ( 1.0 - _OcclusionStrength );
				float AmbientOcclusion94 = ( saturate( ( 1.0 - ( 1.0 - blendOpSrc2_g586 ) * ( 1.0 - blendOpDest2_g586 ) ) ));
				
				float2 uv_CutoutMask = IN.ase_texcoord7.xy * _CutoutMask_ST.xy + _CutoutMask_ST.zw;
				float4 tex2DNode8_g561 = tex2D( _CutoutMask, uv_CutoutMask );
				#if defined(_CHANNELSELECTED_RED)
				float staticSwitch30_g561 = tex2DNode8_g561.r;
				#elif defined(_CHANNELSELECTED_GREEN)
				float staticSwitch30_g561 = tex2DNode8_g561.g;
				#elif defined(_CHANNELSELECTED_BLUE)
				float staticSwitch30_g561 = tex2DNode8_g561.b;
				#elif defined(_CHANNELSELECTED_ALPHA)
				float staticSwitch30_g561 = tex2DNode8_g561.a;
				#else
				float staticSwitch30_g561 = tex2DNode8_g561.r;
				#endif
				float clampResult24_g561 = clamp( (( _InvertCutout )?( ( 1.0 - (( _UseBaseColorAlpha )?( BaseColorAlpha46 ):( staticSwitch30_g561 )) ) ):( (( _UseBaseColorAlpha )?( BaseColorAlpha46 ):( staticSwitch30_g561 )) )) , 0.0 , 1.0 );
				float temp_output_35_0_g561 = (-0.65 + (_IntensityCutoutMap1 - 0.0) * (0.65 - -0.65) / (1.0 - 0.0));
				float Cutout370 = (( _EnableAlphaClipping )?( ( clampResult24_g561 + temp_output_35_0_g561 ) ):( 1.0 ));
				
				float CutOff408 = _IntensityCutoutMap;
				...
				float AlphaClipThreshold = CutOff408;
