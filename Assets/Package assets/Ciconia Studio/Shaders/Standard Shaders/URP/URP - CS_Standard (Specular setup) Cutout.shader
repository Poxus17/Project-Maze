Shader "Ciconia Studio/CS_Standard/URP/Standard (Specular setup)/Cutout"
{
	Properties
	{
		...
		[Space(15)][KeywordEnum(Red,Green,Blue,Alpha)] _ChannelSelection1("Channel Selection", Float) = 0
	...
	SubShader
	{
		...
		Pass
		{
			...
			UNITY_INSTANCING_BUFFER_START(CiconiaStudioCS_StandardURPStandardSpecularsetupCutout)
			...
			UNITY_INSTANCING_BUFFER_END(CiconiaStudioCS_StandardURPStandardSpecularsetupCutout)
			...
			half4 frag ( VertexOutput IN , half ase_vface : VFACE ) : SV_Target
			{
				...
				float4 _BaseMap_ST_Instance = UNITY_ACCESS_INSTANCED_PROP(CiconiaStudioCS_StandardURPStandardSpecularsetupCutout,_BaseMap_ST);
				float2 uv_BaseMap = IN.ase_texcoord7.xy * _BaseMap_ST_Instance.xy + _BaseMap_ST_Instance.zw;
				float2 break26_g649 = uv_BaseMap;
				float GlobalTilingX11 = ( _GlobalXYTilingXYZWOffsetXY.x - 1.0 );
				float GlobalTilingY8 = ( _GlobalXYTilingXYZWOffsetXY.y - 1.0 );
				float2 appendResult14_g649 = (float2(( break26_g649.x * GlobalTilingX11 ) , ( break26_g649.y * GlobalTilingY8 )));
				float GlobalOffsetX10 = _GlobalXYTilingXYZWOffsetXY.z;
				float GlobalOffsetY9 = _GlobalXYTilingXYZWOffsetXY.w;
				float2 appendResult13_g649 = (float2(( break26_g649.x + GlobalOffsetX10 ) , ( break26_g649.y + GlobalOffsetY9 )));
				float4 _ParallaxMap_ST_Instance = UNITY_ACCESS_INSTANCED_PROP(CiconiaStudioCS_StandardURPStandardSpecularsetupCutout,_ParallaxMap_ST);
				...
				float2 paralaxOffset36_g468 = ParallaxOffset( temp_cast_0.x , _Parallax , ase_tanViewDir );
				float2 switchResult47_g468 = (((ase_vface>0)?(paralaxOffset36_g468):(0.0)));
				float2 Parallaxe369 = switchResult47_g468;
				float4 tex2DNode7_g648 = tex2D( _BaseMap, ( ( appendResult14_g649 + appendResult13_g649 ) + Parallaxe369 ) );
				float clampResult27_g648 = clamp( _Saturation , -1.0 , 100.0 );
				float3 desaturateInitialColor29_g648 = ( _Color * tex2DNode7_g648 ).rgb;
				float desaturateDot29_g648 = dot( desaturateInitialColor29_g648, float3( 0.299, 0.587, 0.114 ));
				float3 desaturateVar29_g648 = lerp( desaturateInitialColor29_g648, desaturateDot29_g648.xxx, -clampResult27_g648 );
				float4 temp_output_422_0 = CalculateContrast(_Brightness,float4( desaturateVar29_g648 , 0.0 ));
				float4 _DetailAlbedoMap_ST_Instance = UNITY_ACCESS_INSTANCED_PROP(CiconiaStudioCS_StandardURPStandardSpecularsetupCutout,_DetailAlbedoMap_ST);
				...
				float2 texCoord390 = IN.ase_texcoord7.xy * float2( 1,1 ) + float2( 0,0 );
				float4 tex2DNode395 = tex2D( _DetailMask, texCoord390 );
				#if defined(_CHANNELSELECTIONMASK1_RED)
				float staticSwitch429 = tex2DNode395.r;
				#elif defined(_CHANNELSELECTIONMASK1_GREEN)
				float staticSwitch429 = tex2DNode395.g;
				#elif defined(_CHANNELSELECTIONMASK1_BLUE)
				float staticSwitch429 = tex2DNode395.b;
				#elif defined(_CHANNELSELECTIONMASK1_ALPHA)
				float staticSwitch429 = tex2DNode395.a;
				#else
				float staticSwitch429 = tex2DNode395.r;
				#endif
				float2 appendResult394 = (float2(_TriplanarXYTilingXYZWOffsetXY1.x , _TriplanarXYTilingXYZWOffsetXY1.y));
				float _TriplanarFalloff_Instance = UNITY_ACCESS_INSTANCED_PROP(CiconiaStudioCS_StandardURPStandardSpecularsetupCutout,_TriplanarFalloff);
				#if defined(_TRIPLANARSPACEPROJECTION_OBJECTSPACE)
				float3 staticSwitch389 = IN.ase_texcoord8.xyz;
				#elif defined(_TRIPLANARSPACEPROJECTION_WORLDSPACE)
				float3 staticSwitch389 = WorldPosition;
				#else
				float3 staticSwitch389 = IN.ase_texcoord8.xyz;
				#endif
				float2 appendResult388 = (float2(_TriplanarXYTilingXYZWOffsetXY1.z , _TriplanarXYTilingXYZWOffsetXY1.w));
				float4 triplanar396 = TriplanarSampling396( _DetailMask, ( staticSwitch389 + float3( appendResult388 ,  0.0 ) ), IN.ase_normal, _TriplanarFalloff_Instance, appendResult394, 1.0, 0 );
				#if defined(_CHANNELSELECTION1_RED)
				float staticSwitch428 = triplanar396.x;
				#elif defined(_CHANNELSELECTION1_GREEN)
				float staticSwitch428 = triplanar396.y;
				#elif defined(_CHANNELSELECTION1_BLUE)
				float staticSwitch428 = triplanar396.z;
				#elif defined(_CHANNELSELECTION1_ALPHA)
				float staticSwitch428 = triplanar396.w;
				#else
				float staticSwitch428 = triplanar396.x;
				#endif
				float4 temp_cast_12 = ((( _InvertMask )?( ( 1.0 - (( _EnableTriplanarProjection )?( staticSwitch428 ):( staticSwitch429 )) ) ):( (( _EnableTriplanarProjection )?( staticSwitch428 ):( staticSwitch429 )) ))).xxxx;
				float4 clampResult409 = clamp( ( CalculateContrast(( _ContrastDetailMap + 1.0 ),temp_cast_12) + -_SpreadDetailMap ) , float4( 0,0,0,0 ) , float4( 1,1,1,0 ) );
				float MaskIntensity408 = _IntensityMask;
				float4 Mask158 = (( _EnableDetailMask )?( ( clampResult409 * MaskIntensity408 ) ):( temp_cast_6 ));
				float4 lerpResult343 = lerp( temp_output_422_0 , AlbedoDetail153 , Mask158);
				float4 blendOpSrc15_g650 = temp_output_422_0;
				float4 blendOpDest15_g650 = lerpResult343;
				float4 lerpBlendMode15_g650 = lerp(blendOpDest15_g650,(( blendOpDest15_g650 > 0.5 ) ? ( 1.0 - 2.0 * ( 1.0 - blendOpDest15_g650 ) * ( 1.0 - blendOpSrc15_g650 ) ) : ( 2.0 * blendOpDest15_g650 * blendOpSrc15_g650 ) ),Mask158.x);
				#if defined(_BLENDMODE_NONE)
				float4 staticSwitch22_g650 = lerpResult343;
				#elif defined(_BLENDMODE_OVERLAY)
				float4 staticSwitch22_g650 = ( saturate( lerpBlendMode15_g650 ));
				#else
				float4 staticSwitch22_g650 = lerpResult343;
				#endif
				float4 Albedo26 = (( _VisualizeMask )?( Mask158 ):( staticSwitch22_g650 ));
				
				float4 _BumpMap_ST_Instance = UNITY_ACCESS_INSTANCED_PROP(CiconiaStudioCS_StandardURPStandardSpecularsetupCutout,_BumpMap_ST);
				float2 uv_BumpMap = IN.ase_texcoord7.xy * _BumpMap_ST_Instance.xy + _BumpMap_ST_Instance.zw;
				float2 break26_g699 = uv_BumpMap;
				float2 appendResult14_g699 = (float2(( break26_g699.x * GlobalTilingX11 ) , ( break26_g699.y * GlobalTilingY8 )));
				float2 appendResult13_g699 = (float2(( break26_g699.x + GlobalOffsetX10 ) , ( break26_g699.y + GlobalOffsetY9 )));
				float3 unpack4_g698 = UnpackNormalScale( tex2D( _BumpMap, ( ( appendResult14_g699 + appendResult13_g699 ) + Parallaxe369 ) ), _BumpScale );
				unpack4_g698.z = lerp( 1, unpack4_g698.z, saturate(_BumpScale) );
				float3 temp_output_423_0 = unpack4_g698;
				float4 _DetailNormalMap_ST_Instance = UNITY_ACCESS_INSTANCED_PROP(CiconiaStudioCS_StandardURPStandardSpecularsetupCutout,_DetailNormalMap_ST);
				float2 uv_DetailNormalMap = IN.ase_texcoord7.xy * _DetailNormalMap_ST_Instance.xy + _DetailNormalMap_ST_Instance.zw;
				float3 unpack4_g690 = UnpackNormalScale( tex2D( _DetailNormalMap, uv_DetailNormalMap ), _DetailNormalMapScale );
				unpack4_g690.z = lerp( 1, unpack4_g690.z, saturate(_DetailNormalMapScale) );
				float3 NormalDetail155 = unpack4_g690;
				float3 lerpResult137 = lerp( temp_output_423_0 , NormalDetail155 , Mask158.rgb);
				float3 lerpResult205 = lerp( temp_output_423_0 , BlendNormal( temp_output_423_0 , NormalDetail155 ) , Mask158.rgb);
				float3 Normal27 = (( _BlendMainNormal )?( lerpResult205 ):( lerpResult137 ));
				
				float4 _EmissionMap_ST_Instance = UNITY_ACCESS_INSTANCED_PROP(CiconiaStudioCS_StandardURPStandardSpecularsetupCutout,_EmissionMap_ST);
				float2 uv_EmissionMap = IN.ase_texcoord7.xy * _EmissionMap_ST_Instance.xy + _EmissionMap_ST_Instance.zw;
				float2 break26_g692 = uv_EmissionMap;
				float2 appendResult14_g692 = (float2(( break26_g692.x * GlobalTilingX11 ) , ( break26_g692.y * GlobalTilingY8 )));
				float2 appendResult13_g692 = (float2(( break26_g692.x + GlobalOffsetX10 ) , ( break26_g692.y + GlobalOffsetY9 )));
				float4 temp_output_426_0 = ( _EmissionColor * tex2D( _EmissionMap, ( ( appendResult14_g692 + appendResult13_g692 ) + Parallaxe369 ) ) * _EmissionIntensity );
				float4 temp_cast_22 = (0.0).xxxx;
				float4 lerpResult190 = lerp( temp_output_426_0 , temp_cast_22 , Mask158);
				float4 Emission110 = (( _UseEmissionFromMainProperties )?( temp_output_426_0 ):( lerpResult190 ));
				
				float4 _SpecGlossMap_ST_Instance = UNITY_ACCESS_INSTANCED_PROP(CiconiaStudioCS_StandardURPStandardSpecularsetupCutout,_SpecGlossMap_ST);
				float2 uv_SpecGlossMap = IN.ase_texcoord7.xy * _SpecGlossMap_ST_Instance.xy + _SpecGlossMap_ST_Instance.zw;
				float2 break26_g694 = uv_SpecGlossMap;
				float2 appendResult14_g694 = (float2(( break26_g694.x * GlobalTilingX11 ) , ( break26_g694.y * GlobalTilingY8 )));
				float2 appendResult13_g694 = (float2(( break26_g694.x + GlobalOffsetX10 ) , ( break26_g694.y + GlobalOffsetY9 )));
				float4 tex2DNode3_g693 = tex2D( _SpecGlossMap, ( ( appendResult14_g694 + appendResult13_g694 ) + Parallaxe369 ) );
				float4 _DetailSpecGlossMap_ST_Instance = UNITY_ACCESS_INSTANCED_PROP(CiconiaStudioCS_StandardURPStandardSpecularsetupCutout,_DetailSpecGlossMap_ST);
				float2 uv_DetailSpecGlossMap = IN.ase_texcoord7.xy * _DetailSpecGlossMap_ST_Instance.xy + _DetailSpecGlossMap_ST_Instance.zw;
				float4 tex2DNode3_g695 = tex2D( _DetailSpecGlossMap, uv_DetailSpecGlossMap );
				float4 DetailMetallic176 = ( _DetailSpecularColor * tex2DNode3_g695 * _DetailSpecularIntensity );
				float4 lerpResult179 = lerp( ( _SpecularColor * tex2DNode3_g693 * _SpecularIntensity ) , DetailMetallic176 , Mask158);
				float4 Specular41 = lerpResult179;
				
				float BaseColorAlpha46 = (( _InvertABaseColor )?( ( 1.0 - tex2DNode7_g648.a ) ):( tex2DNode7_g648.a ));
				#if defined(_SOURCE_SPECULARALPHA)
				float staticSwitch31_g693 = ( tex2DNode3_g693.a * _Smoothness );
				#elif defined(_SOURCE_BASECOLORALPHA)
				float staticSwitch31_g693 = ( _Smoothness * BaseColorAlpha46 );
				#else
				float staticSwitch31_g693 = ( tex2DNode3_g693.a * _Smoothness );
				#endif
				float DetailBaseColorAlpha170 = tex2DNode7_g612.a;
				#if defined(_DETAILSOURCE_SPECULARALPHA)
				float staticSwitch31_g695 = ( tex2DNode3_g695.a * _DetailGlossiness );
				#elif defined(_DETAILSOURCE_ALBEDOALPHA)
				float staticSwitch31_g695 = ( _DetailGlossiness * DetailBaseColorAlpha170 );
				#else
				float staticSwitch31_g695 = ( tex2DNode3_g695.a * _DetailGlossiness );
				#endif
				float DetailSmoothness175 = staticSwitch31_g695;
				float lerpResult182 = lerp( staticSwitch31_g693 , DetailSmoothness175 , Mask158.r);
				float Smoothness40 = lerpResult182;
				
				float4 _OcclusionMap_ST_Instance = UNITY_ACCESS_INSTANCED_PROP(CiconiaStudioCS_StandardURPStandardSpecularsetupCutout,_OcclusionMap_ST);
				float2 uv_OcclusionMap = IN.ase_texcoord7.xy * _OcclusionMap_ST_Instance.xy + _OcclusionMap_ST_Instance.zw;
				float2 break26_g697 = uv_OcclusionMap;
				...
				float blendOpSrc2_g696 = tex2D( _OcclusionMap, ( ( appendResult14_g697 + appendResult13_g697 ) + Parallaxe369 ) ).r;
				float blendOpDest2_g696 = ( 1.0 - _OcclusionStrength );
				float temp_output_425_0 = ( saturate( ( 1.0 - ( 1.0 - blendOpSrc2_g696 ) * ( 1.0 - blendOpDest2_g696 ) ) ));
				float lerpResult185 = lerp( temp_output_425_0 , 1.0 , Mask158.r);
				float AmbientOcclusion94 = (( _UseAoFromMainProperties )?( temp_output_425_0 ):( lerpResult185 ));
				
				float4 _CutoutMask_ST_Instance = UNITY_ACCESS_INSTANCED_PROP(CiconiaStudioCS_StandardURPStandardSpecularsetupCutout,_CutoutMask_ST);
				float2 uv_CutoutMask = IN.ase_texcoord7.xy * _CutoutMask_ST_Instance.xy + _CutoutMask_ST_Instance.zw;
				float4 tex2DNode8_g658 = tex2D( _CutoutMask, uv_CutoutMask );
				#if defined(_CHANNELSELECTED_RED)
				float staticSwitch30_g658 = tex2DNode8_g658.r;
				#elif defined(_CHANNELSELECTED_GREEN)
				float staticSwitch30_g658 = tex2DNode8_g658.g;
				#elif defined(_CHANNELSELECTED_BLUE)
				float staticSwitch30_g658 = tex2DNode8_g658.b;
				#elif defined(_CHANNELSELECTED_ALPHA)
				float staticSwitch30_g658 = tex2DNode8_g658.a;
				#else
				float staticSwitch30_g658 = tex2DNode8_g658.r;
				#endif
				float clampResult24_g658 = clamp( (( _InvertCutout )?( ( 1.0 - (( _UseBaseColorAlpha )?( BaseColorAlpha46 ):( staticSwitch30_g658 )) ) ):( (( _UseBaseColorAlpha )?( BaseColorAlpha46 ):( staticSwitch30_g658 )) )) , 0.0 , 1.0 );
				float temp_output_35_0_g658 = (-0.65 + (_IntensityCutoutMap1 - 0.0) * (0.65 - -0.65) / (1.0 - 0.0));
				float Cutout352 = (( _EnableAlphaClipping )?( ( clampResult24_g658 + temp_output_35_0_g658 ) ):( 1.0 ));
				
				float CutOff420 = _IntensityCutoutMap;
				...
				float Alpha = Cutout352;
				float AlphaClipThreshold = CutOff420;
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN , half ase_vface : VFACE ) : SV_TARGET
			{
				...
				float _TriplanarFalloff_Instance = UNITY_ACCESS_INSTANCED_PROP(CiconiaStudioCS_StandardURPStandardSpecularsetupCutout,_TriplanarFalloff);
				#if defined(_TRIPLANARSPACEPROJECTION_OBJECTSPACE)
				float3 staticSwitch389 = IN.ase_texcoord6.xyz;
				#elif defined(_TRIPLANARSPACEPROJECTION_WORLDSPACE)
				float3 staticSwitch389 = WorldPosition;
				#else
				float3 staticSwitch389 = IN.ase_texcoord6.xyz;
				...
				float4 lerpResult190 = lerp( temp_output_426_0 , temp_cast_20 , Mask158);
				float4 Emission110 = (( _UseEmissionFromMainProperties )?( temp_output_426_0 ):( lerpResult190 ));
