Shader "Ciconia Studio/CS_Standard/URP/Standard/Opaque"
{
	Properties
	{
		...
		[ASEEnd][Toggle]_UseEmissionFromMainProperties("Use Emission From Main Properties", Float) = 1
	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag ( VertexOutput IN , half ase_vface : VFACE ) : SV_Target
			{
				...
				float2 break26_g642 = uv_BaseMap;
				float GlobalTilingX11 = ( _GlobalXYTilingXYZWOffsetXY.x - 1.0 );
				float GlobalTilingY8 = ( _GlobalXYTilingXYZWOffsetXY.y - 1.0 );
				float2 appendResult14_g642 = (float2(( break26_g642.x * GlobalTilingX11 ) , ( break26_g642.y * GlobalTilingY8 )));
				float GlobalOffsetX10 = _GlobalXYTilingXYZWOffsetXY.z;
				float GlobalOffsetY9 = _GlobalXYTilingXYZWOffsetXY.w;
				float2 appendResult13_g642 = (float2(( break26_g642.x + GlobalOffsetX10 ) , ( break26_g642.y + GlobalOffsetY9 )));
				...
				float4 tex2DNode7_g641 = tex2D( _BaseMap, ( ( appendResult14_g642 + appendResult13_g642 ) + Parallaxe376 ) );
				float clampResult27_g641 = clamp( _Saturation , -1.0 , 100.0 );
				float3 desaturateInitialColor29_g641 = ( _BaseColor * tex2DNode7_g641 ).rgb;
				float desaturateDot29_g641 = dot( desaturateInitialColor29_g641, float3( 0.299, 0.587, 0.114 ));
				float3 desaturateVar29_g641 = lerp( desaturateInitialColor29_g641, desaturateDot29_g641.xxx, -clampResult27_g641 );
				float4 temp_output_481_0 = CalculateContrast(_Brightness,float4( desaturateVar29_g641 , 0.0 ));
				...
				float2 texCoord453 = IN.ase_texcoord7.xy * float2( 1,1 ) + float2( 0,0 );
				...
				#elif defined(_CHANNELSELECTIONMASK1_GREEN)
				float staticSwitch492 = triplanar459.y;
				#elif defined(_CHANNELSELECTIONMASK1_BLUE)
				float staticSwitch492 = triplanar459.z;
				#elif defined(_CHANNELSELECTIONMASK1_ALPHA)
				float staticSwitch492 = triplanar459.w;
				#else
				float staticSwitch492 = triplanar459.x;
				#endif
				float4 temp_cast_12 = ((( _InvertMask )?( ( 1.0 - (( _EnableTriplanarProjection )?( staticSwitch492 ):( staticSwitch493 )) ) ):( (( _EnableTriplanarProjection )?( staticSwitch492 ):( staticSwitch493 )) ))).xxxx;
				float4 clampResult473 = clamp( ( CalculateContrast(( _ContrastDetailMap + 1.0 ),temp_cast_12) + -_SpreadDetailMap ) , float4( 0,0,0,0 ) , float4( 1,1,1,0 ) );
				float MaskIntensity471 = _IntensityMask;
				float4 Mask158 = (( _EnableDetailMask )?( ( clampResult473 * MaskIntensity471 ) ):( temp_cast_6 ));
				float4 lerpResult343 = lerp( temp_output_481_0 , AlbedoDetail153 , Mask158);
				float4 blendOpSrc15_g643 = temp_output_481_0;
				...
				float2 break26_g666 = uv_BumpMap;
				...
				float3 unpack4_g665 = UnpackNormalScale( tex2D( _BumpMap, ( ( appendResult14_g666 + appendResult13_g666 ) + Parallaxe376 ) ), _BumpScale );
				unpack4_g665.z = lerp( 1, unpack4_g665.z, saturate(_BumpScale) );
				float3 temp_output_488_0 = unpack4_g665;
				...
				float3 lerpResult137 = lerp( temp_output_488_0 , NormalDetail155 , Mask158.rgb);
				float3 lerpResult205 = lerp( temp_output_488_0 , BlendNormal( temp_output_488_0 , NormalDetail155 ) , Mask158.rgb);
				...
				float2 break26_g673 = uv_EmissionMap;
				...
				float4 temp_output_491_0 = ( _EmissionColor * tex2D( _EmissionMap, ( ( appendResult14_g673 + appendResult13_g673 ) + Parallaxe376 ) ) * _EmissionIntensity );
				float4 temp_cast_22 = (0.0).xxxx;
				float4 lerpResult190 = lerp( temp_output_491_0 , temp_cast_22 , Mask158);
				float4 Emission110 = (( _UseEmissionFromMainProperties )?( temp_output_491_0 ):( lerpResult190 ));
				...
				float2 break26_g671 = uv_MetallicGlossMap;
				float2 appendResult14_g671 = (float2(( break26_g671.x * GlobalTilingX11 ) , ( break26_g671.y * GlobalTilingY8 )));
				float2 appendResult13_g671 = (float2(( break26_g671.x + GlobalOffsetX10 ) , ( break26_g671.y + GlobalOffsetY9 )));
				float4 tex2DNode3_g670 = tex2D( _MetallicGlossMap, ( ( appendResult14_g671 + appendResult13_g671 ) + Parallaxe376 ) );
				...
				float4 tex2DNode3_g667 = tex2D( _DetailMetallicGlossMap, uv_DetailMetallicGlossMap );
				float DetailMetallic176 = ( tex2DNode3_g667.r * _DetailMetallic );
				float lerpResult179 = lerp( ( tex2DNode3_g670.r * _Metallic ) , DetailMetallic176 , Mask158.r);
				float Metallic41 = lerpResult179;
				
				float BaseColorAlpha46 = (( _InvertABaseColor1 )?( ( 1.0 - tex2DNode7_g641.a ) ):( tex2DNode7_g641.a ));
				#if defined(_SOURCE_METALLICALPHA)
				float staticSwitch23_g670 = ( tex2DNode3_g670.a * _Smoothness );
				#elif defined(_SOURCE_BASECOLORALPHA)
				float staticSwitch23_g670 = ( _Smoothness * BaseColorAlpha46 );
				#else
				float staticSwitch23_g670 = ( tex2DNode3_g670.a * _Smoothness );
				#endif
				float DetailBaseColorAlpha170 = tex2DNode7_g612.a;
				#if defined(_DETAILSOURCE_METALLICALPHA)
				float staticSwitch23_g667 = ( tex2DNode3_g667.a * _DetailGlossiness );
				#elif defined(_DETAILSOURCE_BASECOLORALPHA)
				float staticSwitch23_g667 = ( _DetailGlossiness * DetailBaseColorAlpha170 );
				#else
				float staticSwitch23_g667 = ( tex2DNode3_g667.a * _DetailGlossiness );
				#endif
				float DetailSmoothness175 = staticSwitch23_g667;
				float lerpResult182 = lerp( staticSwitch23_g670 , DetailSmoothness175 , Mask158.r);
				...
				float2 break26_g669 = uv_OcclusionMap;
				float2 appendResult14_g669 = (float2(( break26_g669.x * GlobalTilingX11 ) , ( break26_g669.y * GlobalTilingY8 )));
				float2 appendResult13_g669 = (float2(( break26_g669.x + GlobalOffsetX10 ) , ( break26_g669.y + GlobalOffsetY9 )));
				float blendOpSrc2_g668 = tex2D( _OcclusionMap, ( ( appendResult14_g669 + appendResult13_g669 ) + Parallaxe376 ) ).r;
				float blendOpDest2_g668 = ( 1.0 - _OcclusionStrength );
				float temp_output_490_0 = ( saturate( ( 1.0 - ( 1.0 - blendOpSrc2_g668 ) * ( 1.0 - blendOpDest2_g668 ) ) ));
				float lerpResult185 = lerp( temp_output_490_0 , 1.0 , Mask158.r);
				float AmbientOcclusion94 = (( _UseAoFromMainProperties )?( temp_output_490_0 ):( lerpResult185 ));
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN , half ase_vface : VFACE ) : SV_TARGET
			{
				...
				float4 lerpResult190 = lerp( temp_output_491_0 , temp_cast_20 , Mask158);
				float4 Emission110 = (( _UseEmissionFromMainProperties )?( temp_output_491_0 ):( lerpResult190 ));
